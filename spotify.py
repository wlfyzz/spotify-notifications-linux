...
def send_notification(s, b, i):
  subprocess.run(['gdbus', 'call', '--session', '--dest', 'org.freedesktop.Notifications','--object-path', '/org/freedesktop/Notifications', '--method','org.freedesktop.Notifications.Notify', 'Spotify', '42', i, s, b, '[]', '{}', '5000']) # Call freedesktop.Notification 
...
while True:
    try:
        data = (
            requests.get(f"https://api.lanyard.rest/v1/users/{userid}")
            .json()
            .get("data", {})
            .get("spotify", {})
        )
    ...
    if "timestamps" in data:
        title, artist = data.get("song", "Unknown Title"), data.get(
